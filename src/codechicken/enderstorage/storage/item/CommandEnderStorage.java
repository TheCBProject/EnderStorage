...
public class CommandEnderStorage extends PlayerCommand
{
    ...
    public void handleCommand(WorldServer world, EntityPlayerMP player, String[] args)
    {
        ...
        {
            wrapped.addChatMessage(new ChatComponentTranslation("enderstorage.command.no_arguments"));
            return;
        }
        ...
        if(args.length <= 2)
        {
            ...
            {
                wrapped.addChatMessage(new ChatComponentTranslation("enderstorage.command.no_freq"));
                return;
            }
        ...
        else
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                ...
                if(d < 0)
                {
                    wrapped.addChatMessage(new ChatComponentTranslation("enderstorage.command.no_colour", args[i]));
                    return;
                }
                colours[i] = d;
            }
    ...
    {
        listener.addChatMessage(new ChatComponentText(getCommandUsage(listener)));
    }
